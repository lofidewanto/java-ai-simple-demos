...
This document covers all application configuration properties, developer tooling URLs, build instructions, and run instructions needed to get the Order Processing application running from a fresh clone.
...
```
target/demo-sdd-noworkflow-spring-0.0.1-SNAPSHOT.jar
```
...
2. Flyway runs pending migrations against the H2 in-memory database (`V1__Create_orders_table.sql`).
3. Hibernate validates the schema against the entity mappings.
4. `DataInitializer` (`CommandLineRunner`) seeds 3 sample orders and applies workflow transitions.
...
```properties
# ── Application ────────────────────────────────────────────────
spring.application.name=demo-sdd-noworkflow-spring
...
|---|---|---|
| `spring.datasource.url` | `jdbc:h2:mem:orderdb` | In-memory H2 database named `orderdb`. Destroyed on shutdown. |
...
| `spring.jpa.hibernate.ddl-auto` | `validate` | Hibernate only validates schema; never creates or alters tables. Schema owned by Flyway. |
...
| Property | Value | Description |
|---|---|---|
| `spring.thymeleaf.cache` | `false` | Disables template caching. Changes to `.html` files reflected without restart (requires DevTools). **Set to `true` in production.** |
...
| URL | Description |
|---|---|
| `http://localhost:8080/` | Root — redirects to `/orders` |
...
DevTools provides:
- **Automatic restart** when class files on the classpath change.
- **LiveReload** integration — browser refreshes automatically on static resource changes.
- **Thymeleaf cache disabled** by default in dev.

DevTools is **excluded from the packaged JAR** by `spring-boot-maven-plugin`.
...
Packages the application as an executable fat JAR:
...
Enables the Spring Boot Configuration Processor annotation processor:
